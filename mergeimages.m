function [Reached_Pattern,Global_image,Central_Xposition,Central_Yposition] = mergeimages(Maximum_intensity,O,EstimatedNumber,xx,yy,xpixel,ypixel,num)
...
%         imshow(Global_image,[]);colormap hot;
        ...
        %figg=figure(100);
        hh = histogram(O{Reached_Pattern(k)},70);
        zz = hh.Values;
        background = hh.BinEdges(find(zz == max(zz))+1);
        %close(figg);
        %find(zz == max(zz));
        %global_image = global_image*max(max(background));
        %global_image = global_image*min(min(background));
        %if size(global_image)==size(background)
        global_image = global_image*background;
        if(((Central_Xposition(Reached_Pattern(k))-floor(xpixel/2))>0)&&((Central_Yposition(Reached_Pattern(k))-floor(xpixel/2))>0)...
            &&((Central_Xposition(Reached_Pattern(k))+floor(xpixel/2))-1<510)&&((Central_Yposition(Reached_Pattern(k))+floor(xpixel/2)-1)<510))
            ...
            = O{Reached_Pattern(k)};
        %en
       % if size(global_image)==size(Global_image)
        Global_image = Global_image + global_image; % merge them into the blobal image.
       % end
        end
        ff = ff+1;
